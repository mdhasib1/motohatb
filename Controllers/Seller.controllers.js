...
const getToken = async () => {
  try {
    const response = await fetch(
      `${process.env.PATHAO_API_ENDPOINT}/aladdin/api/v1/issue-token`,
      {
        method: "POST",
        headers: {
...
const SellerController = {
  register: async (req, res, io) => {
    try {
      ...
      // Check for existing seller by email or phone
      const existingSeller = await User.findOne({
        $or: [{ email: seller_email }, { phone: seller_mobile }],
      });
      const existingSellerBusinessName = await User.findOne({
        seller_business_name: seller_business_name,
      });
      if (existingSellerBusinessName) {
        return res
          .status(400)
          .json({ message: "Seller Business Name already exists" });
      }
      if (existingSeller) {
        return res
          .status(400)
      ...
      // Create a new seller (User) entry
      ...
      // Fetch token and register the store with the external system (e.g., Pathao)
      const token = await getToken();
      const response = await fetch(
        `${process.env.PATHAO_API_ENDPOINT}/aladdin/api/v1/stores`,
        {
          method: "POST",
          headers: {
      ...
      // Create a new store entry in your database
      const newStore = new Store({
        name: storeData.data.store_name,
  ...
  login: async (req, res, io) => {
    try {
      ...
      res
        .status(401)
        .json({ error: "Invalid credentials or internal server error." });
    }
  },
  ...
  updateProfile: async (req, res, io) => {
    try {
      ...
      const seller = await Seller.findByIdAndUpdate(sellerId, updatedProfile, {
        new: true,
      });
  ...
  sendPasswordResetEmail: async (req, res) => {
    try {
      ...
      res
        ...
        .json({
          success: false,
          message: "Instructions not sent, please try again",
        });
    }
  },
  resendOtp: async (req, res) => {
    try {
      ...
      if (method === "phone" && user.phone) {
        ...
        res
          .status(400)
          .json({ message: "User has no phone or email specified." });
      }
    } catch (error) {
  ...
  getStores: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "All Stores get successfully", storeData });
    } catch (error) {}
  },
};
