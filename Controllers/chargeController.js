...
const getChargeById = async (req, res) => {
  try {
    const { id } = req.params;
    const charge = await Charge.findById(id).populate('subCategory');
...
const deleteCharge = async (req, res) => {
  ...
  const getAllCharges = async (req, res) => {
    const { id } = req.params;
    console.log('here is sub category id' , id)
    try {
      const charges = await Charge.find({ subCategory: id }).populate('subCategory');
